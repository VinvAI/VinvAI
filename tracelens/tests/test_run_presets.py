...
def test_default_is_standard(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_full_matches_standard_axes(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("TRACELENS_MEMORY", raising=False)
...
def test_granular_flag_overrides_preset_per_axis(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("TRACELENS_MEMORY", raising=False)
    monkeypatch.delenv("TRACELENS_CAPTURE_DETERMINISM", raising=False)
    # standard, but strip memory + determinism individually
    memory, determinism = _resolve(["-t", "myapp", "--no-memory", "--no-capture-determinism"])
    assert memory is False
    assert determinism is False
...
def test_env_memory_override(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("TRACELENS_CAPTURE_DETERMINISM", raising=False)
    monkeypatch.setenv("TRACELENS_MEMORY", "0")
    memory, _determinism = _resolve(["-t", "myapp"])
    assert memory is False  # env override beats standard preset default
