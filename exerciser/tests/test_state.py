    ...
    vals = state.scalar_values(
        {
            ...
            "n": 12345,
            "ok": True,
            "tag": "ab",  # number/bool/short → dropped
...
def _exec_row(method="POST", path="/users/signup", status=200, input=None, body=None):
    return {
...
def test_teardown_stamps_attempt_counts_and_status():
    ...
    state.attempt_teardown(creation, endpoints, "http://x", lambda *a, **k: _probe(404))
    ...
    state.attempt_teardown(creation, endpoints, "http://x", lambda *a, **k: _probe(200))
        ...
        [
            {
                ...
                "method": "POST",
                "path": "/users/signup",
                "planted": ["stale@x.test"],
                "response_values": ["stale-uuid-1"],
                "cleaned": False,
                "teardown_attempts": 1,
                "teardown_status": "failed",
            },
            {
                "endpoint_id": "POST_done",
                "method": "POST",
                "path": "/users/signup",
                "planted": ["done@x.test"],
                "response_values": ["done-uuid-9"],
                "cleaned": True,
            },
        ],
    )
        ...
        [
            {
                ...
                "method": "POST",
                "path": "/users/signup",
                "planted": ["stuck@x.test"],
                "response_values": ["stuck-uuid-2"],
                "cleaned": False,
                "teardown_attempts": 1,
                "teardown_status": "failed",
            },
...
def test_mark_expired_setup_failure_and_auth_bounce():
    ...
    legit_4xx = _scenario(
        ...
        planned=1,
    )  # endpoint step failed on its own merits — NOT expiry
...
def test_user_authored_reply_feeds_plan_for_non_semantic_endpoint(tmp_path):
    ...
    store.write_json(
        store.prompts_dir(tmp_path) / "GET_health.json",
        {
            "api_id": "GET_health",
            "reply": {
                "plans": [
