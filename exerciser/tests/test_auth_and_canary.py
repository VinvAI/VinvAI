...
def _endpoint(api_id="DELETE_users_id", method="DELETE", path="/users/{user_id}", **kw):
    return {
        "api_id": api_id,
        "method": method,
        "path": path,
        ...
        "inputs": [
            {
                ...
                "class": "valid",
                "body": None,
                "path_params": {"user_id": "generated-uuid"},
                "query": {},
            }
        ],
        **kw,
...
def test_auth_sweep_without_credentials_is_a_noop():
    assert (
        _auth_sweep([_endpoint()], "http://x", "t", lambda *a, **k: _probe(), [], ["id"], LOG) == []
    )
...
def test_canary_passes_on_healthy_setup():
    canary = _environment_canary(
        [_scenario_endpoint(200)],
        "http://x",
        "t",
        lambda *a, **k: _probe(200, {"access_token": "t"}),
        LOG,
    )
...
def test_regress_skips_authed_cases_when_no_setup_available(tmp_path):
    ...
    summary = replay_suite(
        tmp_path,
        "http://x",
        probe_fn=lambda *a, **k: _probe(401),
    )
